...
patch_size = 64;
data = 'sample_'+str(patch_size)
# base_folder = '/home/cxd_macau/'
base_folder = '/Users/oishii/Dataset/tifs-database/'
...
original_images = [os.path.join(base_folder,'DSO-1')+'/'+x for x in os.listdir(os.path.join(base_folder,'DSO-1')) if x.endswith(".png") and "splicing" in x ]
...
for str_type in ['train','val','test']:
    ...
    for img_path in dic[str_type]: # each image in dic
        ...
        rgbmask = Image.open(MASK_PATH).resize(prob.size) # here the mask is in rgb form, we need to change the non-spliced region to 255
        ...
        for h in range(img_patches.shape[0]):
            for w in range(img_patches.shape[1]):
                ...
                im_hole = tmp_patches.transpose(0,2,1,3,4).reshape((64*img_patches.shape[0],64*img_patches.shape[1],3))
                
                im_hole = Image.fromarray(im_hole)

                im_hole = im_hole.resize((224,224),Image.ANTIALIAS)

                im_full = img_patches.transpose(0,2,1,3,4).reshape((64*img_patches.shape[0],64*img_patches.shape[1],3))
                im_full = Image.fromarray(im_full)
                im_full = im_full.resize((224,224),Image.ANTIALIAS)

                PATCH_PATH = os.path.join(data,'images',str_type,str(count)+'.png')
                MASK_PATCH_PATH = os.path.join(data,'masks',str_type,str(count)+'.png')
                HOLE_IMAGE_PATH = os.path.join(data,'hole_probes',str_type,str(count)+'.png')
                FULL_IMAGE_PATH = os.path.join(data,'resized_probes',str_type,str(count)+'.png')
                ...
                lst.append(",".join([PATCH_PATH,MASK_PATCH_PATH,FULL_IMAGE_PATH,MASK_PATH.replace(base_folder,''),IMG_PATH.replace(base_folder,''),HOLE_IMAGE_PATH]))
